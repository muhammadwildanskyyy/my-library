...
class BookService:
    ...
    async def update_book(
        ...
        updated_book = await self._book_repo.update_book(
            book_id, user_id, update_data
        )
    ...
    async def get_book_chunks(
        ...
        """
        Return all text chunks for a book in order.

        Verifies book ownership first, then fetches chunks from the
        PGVector embedding table.

        Args:
            book_id: The book to retrieve chunks for.
            user_id: Current user (ownership enforced).

        Returns:
            Ordered list of chunk dicts.

        Raises:
            BookNotFoundError: If the book is not found or not owned by user.
        """
        # Verify ownership
        book = await self._book_repo.get_by_id(book_id, user_id)
        if not book:
        ...
        return await self._book_repo.get_chunks_by_book_id(book_id, user_id)
